...
function contactLink(name, link) {
  ...
  return (
    <a href={link} style={{margin: '0px 2vw'}}>
...
function Contact() {
  ...
  return (
    <Navbar style={contactStyle} bg="dark" variant="dark" sticky="bottom" id="contact">
      <Col>
        <Row><h3 style={{margin: '0px auto', height: '6vw'}}>Contact</h3></Row>
        <Row><div style={{margin: '0px auto', height: '8vw'}}>{contacts.map(c => contactLink(c.name, c.link))}</div></Row>
      </Col>
    </Navbar>
    );
}
